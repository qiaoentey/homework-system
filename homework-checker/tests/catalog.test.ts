...
describe("answer catalog", () => {
  ...
  it("keeps the complete external-video inventory intentional and status-labelled", () => {
    ...
    expect(inventory).toEqual([
      "1-chinese|下册完整版（补充）|42:20|G9HVvzhPtTk",
      "1-chinese|上册完整版（补充）|42:21|nTLnzUXU7zk",
      "1-chinese|下册逐页核对版|3:51|681yp19nZBM",
      "1-chinese|上册逐页核对版|3:51|OJug4NJxqJw",
      "1-malay|下册完整版（补充）|38:20|ny_RadBlf0g",
      "1-malay|上册完整版（补充）|38:21|P8eEdOz4y38",
      "1-malay|下册逐页核对版|3:31|Keyy5R_NayQ",
      "1-malay|上册逐页核对版|3:31|tWyrRQaLcPU",
      ...
      "2-chinese|下册完整版（补充）|42:21|pV1L8AT0s4w",
      "2-chinese|上册完整版（补充）|42:21|CJf5M2ptlhQ",
      "2-chinese|下册逐页核对版|3:50|AeoMa2D2bzE",
      "2-chinese|上册逐页核对版|3:50|QqxSfx8mNTs",
      "2-malay|下册完整版（补充）|43:21|TQSUQT1NnCk",
      "2-malay|上册完整版（补充）|43:21|X6AAAnHNo9I",
      "2-malay|下册逐页核对版|3:50|3G25_5LBgVc",
      "2-malay|上册逐页核对版|3:50|t45VdtZbe7A",
      ...
      "3-chinese|下册完整版（补充）|43:20|014P1XavOSc",
      "3-chinese|上册完整版（补充）|43:20|XNt5cFUKcFQ",
      "3-chinese|下册逐页核对版|7:30|cqZdp5Zluws",
      "3-chinese|上册逐页核对版|7:30|XYS23HEUw_o",
    ...
    expect(ANSWER_RESOURCES.flatMap((resource) => resource.videos)
      .filter((video) => video.label.includes("完整版") && !video.label.includes("补充"))
      .every((video) => video.primarySource)).toBe(true);
    expect(ANSWER_RESOURCES.flatMap((resource) => resource.videos)
      .filter((video) => video.label.includes("快速版"))
      .every((video) => !video.primarySource)).toBe(true);
    expect(ANSWER_RESOURCES.flatMap((resource) => resource.videos)
      .filter((video) => video.label.includes("逐页核对版"))
      .every((video) => video.primarySource)).toBe(true);
    expect(ANSWER_RESOURCES.flatMap((resource) => resource.videos)
      .filter((video) => video.label.includes("补充"))
      .every((video) => !video.primarySource)).toBe(true);
  });
