...
When a teacher is working inside the MK branch, the student profile school field must show only 一校, 二校, 启智, and 姚贞暖. The school-class field must then show only the classes for the student's selected school and grade.
...
- 姚贞暖: no class options yet; the class field remains blank for every grade.
...
- MK receives a branch-specific school catalog containing only the four approved schools, in this order: 一校、二校、启智、姚贞暖.
...
- Unit tests prove MK has exactly the four schools, the exact class lists for each year, and no classes for 姚贞暖.
